...
function write_results {
	...
	cat logs/"Rx"_$RP.txt | tr -d '+','ns' | gawk '{ sum += $17; i +=1 }; END { print (sum / i) / 10^6 }' > logs/ManhattanGrid/"E2E_rp_"$RP".txt"	
	cat logs/"Rx"_$RP.txt | tr -d '+','ns' | gawk '{ sum += $17; i +=1 }; END { print (sum / i) / 10^6 }'
	echo -e "-------\n"
	
	# Average energy in jouls
	echo -e "*Average Energy*"
	gawk '{ sum += $2; i +=1 }; END { print (sum / i) }' logs/"EN_rp_"$RP".txt" > logs/ManhattanGrid/"ENERGY_rp_"$RP".txt"
	...
	# echo $(( ($PR / $PS) * 100)) > logs/ManhattanGrid/"PDR_rp_"$RP".txt"
	

	PDR=$(bc <<< "scale=4;($PR/$PS)*100")
	echo -e "TX: $PS \nRX: $PR \nPDR: $PDR" 
	echo -e "TX: $PS \nRX: $PR \nPDR: $PDR"  > logs/ManhattanGrid/"PDR_rp_"$RP".txt"

	echo -e "------- **END** ------- \n"
}

# Generates nodes' movement in ManhattanGrid scenarios
...
# Generates Buildings in ManhattanGrid scenarios
...
function set_DIRS {
	...
	# Output
	mkdir -p logs logs/ManhattanGrid
